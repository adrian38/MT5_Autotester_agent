...
from portfolio_manager.mt5_report import (
    RawDeal, _build_trades, _parse_order_stops, _to_float as mt5_to_float,
)
...
class MT5ReportParserTests(unittest.TestCase):
    ...
    def test_stop_comment_links_overlapping_positions_to_the_correct_entry(self) -> None:
        def deal(
            ...
            *, order: str, profit: float = 0.0, comment: str = "",
        ) -> RawDeal:
            return RawDeal(
                timestamp=moment, ticket=ticket, symbol="BTCUSD", trade_type=trade_type,
                direction=direction, volume=0.16, price=price, order=order,
                commission=0.0, swap=0.0, profit=profit, balance=5000.0, comment=comment,
            )
