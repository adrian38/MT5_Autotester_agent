...
from manager_node_runtime.node import RESUMABLE_STATUSES, JobController
...
class ManagerNodePauseResumeTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        self.config = {"node_id": "axi", "project_dir": str(self.root)}
        self.config_path = self.root / "manager_node.json"
    ...
    def test_pause_does_not_send_a_stage_notification(self) -> None:
        # Una pausa pedida no es una etapa terminada: avisar por Telegram seria
        # anunciar un final que no ha ocurrido.
        controller = self.controller()
        process = self.running_pipeline(controller)
        controller.pause()

        with mock.patch.object(controller, "_notify_stage_completion") as notify:
            controller._watch(process, 1)
    ...
    def test_the_node_exposes_the_pause_and_resume_endpoints(self) -> None:
        # El manager llama a estas rutas; sin ellas la interfaz recibe un 404
        # "Ruta no encontrada" al pulsar Pausar.
        source = Path(__file__).resolve().parents[1] / "manager_node_runtime" / "node.py"
        script = source.read_text(encoding="utf-8")
        self.assertIn('self.path == "/api/v1/jobs/pause"', script)
        self.assertIn('self.path == "/api/v1/jobs/resume"', script)
